...
class Item extends Component {
    ...
    handleChange = (e) => {
        ...
        if(e.target.value <= 0) {
            alert("Quantity must be greater than or equal to 1"); // minimum number in cart to work. cart can be empty or 1+ items
 
            return;
        }
 
        if(e.target.value > this.props.item.product.amount) {
            alert("You have exceeded the available items of this product!"); //if product amounts are declared in productReducer
  ...
  render() {
      ...
      return (
          ...
          <div className="row">
              <div className="col-xs-2"><img alt= "desc" className="img-responsive" src={item.product.image} />
              </div>
